...
class JoinerV5(AbsJoiner):
    def __init__(
        self,
        every=0.5,
        threshold=0.0123,
        min_avg_threshold=0.02,
        minimum_min_avg=-2,
        log=False,
    ):
    ...
    def append_context(
        ...
        for i, (start_s, end_s, mavg, rec, start_w, rel_w) in enumerate(self.data):
            if (
                ...
                if (
                    mavg > min_avg
                    # or rel_w > segment_index
                    or abs(mavg - min_avg) < self.MIN_AVG_THRESHOLD
                ):
                    ...
                    self.data[i] = (
                        start_s,
                        end_s,
                        mavg - 0.2 if recognized != rec else mavg,
                        rec,
                        start_w,
                        rel_w,
                    )  # 정확도 조금 낮춰주자
                ...
                if self.log:
                    print(
                        f"update exist token {rec} > {recognized} to abs {st_sec:.2f}s | old_mavg {mavg:.2f} | new_mavg {min_avg:.2f} > {self.get_string()}"
                    )
                return
        ...
        if not (start_window and not segment_index):
            ...
            if self.log:
                print(
                    f"inserted new token {recognized} newly to abs {st_sec:.2f}s | new_mavg {min_avg:.2f} > {self.get_string()}"
                )
